...
//
// Listed explicitly rather than re-exported with `export *`: five of
// complex/'s lowercase factories — `star`, `regularPolygon`,
// `pentagon`, `hexagon`, `isoscelesTriangle` — share a name with the
// vertex-layer Polygon/Triangle factories exported above. A duplicate
// binding made those names resolve one way in TypeScript (the Polygon
// factory) and the other way in bundlers (the shape kind), so
// `star(center, 80, 34, 5)` type-checked and then drew a default 10px
// star at the origin. The shape-kind versions stay reachable through
// their classes (`Star`, `RegularPolygon`, `IsoscelesTriangle`) and by
// name from `allShapes`/`complexShapes`.
export {
  Trapezium,
  trapezium,
  Parallelogram,
