import { picture, nodeRow, point } from 'jikz'
...
export default function render(container: HTMLElement) {
  ...
  const worker = api.rightOf(40, { shape: 'rectangle', text: 'worker' })
  pic.node('worker', { at: worker.center, shape: worker.shape, text: worker.text }, { style: st })

  const queue = api.below(30, { shape: 'cylinder', text: 'queue', width: 54, height: 44 })
  ...
  const days = nodeRow(point(50, 170), ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'], {
    distance: 12,
    nodeOptions: { shape: 'rounded rectangle', shapeOptions: { cornerRadius: 6 } },
  })
  for (const n of days) {
